...
SPORT_TERMS = {
    ...
    "boxing": ["boxing", "fight night", "heavyweight"], "ufc": ["ufc", "ultimate fighting championship"],
    "tennis": ["tennis", "atp", "wta", "grand slam", "wimbledon", "us open", "french open", "australian open"],
    "golf": ["golf", "pga", "lpga", "ryder cup", "masters tournament"], "cricket": ["cricket", "test match", "t20", "odi", "the hundred"],
    "f1": ["formula 1", "formula one", "f1", "grand prix"], "f2": ["formula 2", "f2"], "f3": ["formula 3", "f3"],
    "darts": ["darts", "pdc"], "snooker": ["snooker", "world snooker"], "cycling": ["cycling", "tour de france", "giro d'italia", "vuelta"],
    "motorsport": ["motorsport", "motogp", "nascar", "indycar", "wrc"], "nfl": ["nfl", "super bowl"], "nba": ["nba", "basketball"],
    "baseball": ["mlb", "baseball"], "ice hockey": ["nhl", "ice hockey"], "horse racing": ["horse racing", "racing from", "kempton", "ascot", "cheltenham", "aintree", "newmarket"],
...
def parse_dt(value: str) -> datetime:
    ...
    off = m.group(2)
    if off:
        sign = 1 if off[0] == "+" else -1; minutes = int(off[1:3])*60 + int(off[3:5])
...
def classify(title, desc, channel):
    hay = f"{title} {desc}".lower(); scores = {s: sum(1 for t in ts if t in hay) for s, ts in SPORT_TERMS.items()}; scores = {k:v for k,v in scores.items() if v}
    if scores:
...
def ingest(source_name, urls, start, end):
    ...
    for url in urls:
        try:
            raw=download(url); root=ET.fromstring(xml_bytes(raw,url)); channels={c.attrib.get("id",""):text(c.find("display-name")) for c in root.findall("channel")}; programmes=[]
            for p in root.findall("programme"):
                try: s,e=parse_dt(p.attrib.get("start","")),parse_dt(p.attrib.get("stop",""))
                except Exception: continue
                if e<=start or s>=end: continue
                channel_id=p.attrib.get("channel",""); channel=channels.get(channel_id,channel_id); title,desc=text(p.find("title")),text(p.find("desc")); sport,confidence=classify(title,desc,channel)
                if sport: programmes.append({"channel":channel,"channel_id":channel_id,"title":title,"description":desc,"start":s.isoformat().replace("+00:00","Z"),"end":e.isoformat().replace("+00:00","Z"),"sport":sport,"confidence":confidence,"source":source_name})
            if programmes: return programmes,{"ok":True,"url":url,"programmes":len(programmes),"sha256":hashlib.sha256(raw).hexdigest()}
...
def main():
    ...
    for name,urls in SOURCES.items():
        events,status=ingest(name,urls,start,end); health[name]=status; all_events.extend(events)
    unique={}
    for e in all_events:
    ...
    events=sorted(unique.values(),key=lambda x:x["start"]); payload={"generated_at":now.isoformat().replace("+00:00","Z"),"window_start":start.isoformat().replace("+00:00","Z"),"window_end":end.isoformat().replace("+00:00","Z"),"timezone":"UTC","event_count":len(events),"events":events}
    if events: (OUT/"guide.json").write_text(json.dumps(payload,indent=2),encoding="utf-8"); health["publish"]={"ok":True,"events":len(events)}
    else:
        health["publish"]={"ok":False,"reason":"no valid sports events; previous guide retained if present"}
        if not (OUT/"guide.json").exists(): (OUT/"guide.json").write_text(json.dumps(payload,indent=2),encoding="utf-8")
    (OUT/"health.json").write_text(json.dumps({"checked_at":now.isoformat().replace("+00:00","Z"),"sources":health},indent=2),encoding="utf-8"); print(json.dumps(health,indent=2))
