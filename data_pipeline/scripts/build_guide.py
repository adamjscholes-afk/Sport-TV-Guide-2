...
PROMO_RE = re.compile(r"^(this is |welcome to |channel |coming up|programming on |schedule$)", re.I)
NEWS_RE = re.compile(r"sports news|sky sports news|live at the races", re.I)


def download(url: str) -> bytes:
    req = urllib.request.Request(url, headers={"User-Agent": "SportTVGuide/0.2"})
    ...
    for url in urls:
        try:
            ...
            for p in root.findall("programme"):
                ...
                channel_id=p.attrib.get("channel",""); channel=channels.get(channel_id,channel_id); title,desc=text(p.find("title")),text(p.find("desc")); categories=[text(x) for x in p.findall("category")]; sport,confidence=classify(title,desc,channel,categories)
                if sport: programmes.append({"channel":channel,"channel_id":channel_id,"title":title,"description":desc,"categories":categories,"start":s.isoformat().replace("+00:00","Z"),"end":e.isoformat().replace("+00:00","Z"),"sport":sport,"confidence":confidence,"source":source_name})
...
def main():
    ...
    (OUT/"health.json").write_text(json.dumps({"checked_at":now.isoformat().replace("+00:00","Z"),"window_start":start.isoformat().replace("+00:00","Z"),"window_end":end.isoformat().replace("+00:00","Z"),"sources":health,"channel_count":len(channel_counts),"sport_counts":dict(sorted(sport_counts.items()))},indent=2),encoding="utf-8")
