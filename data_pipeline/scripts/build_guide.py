...
RADIO_CHANNEL_RE=re.compile(r"talksport|talk sport|bbc radio|bbc sounds|radio [0-9]|absolute radio|capital fm|heart fm|kiss fm|smooth fm|lbc|magic radio|virgin radio|greatest hits radio|radio x|classic fm|gold radio",re.I)
BBC_ONE_RE=re.compile(r"\bbbc\s*(?:one|1)\b",re.I); ITV1_RE=re.compile(r"\bitv\s*1\b|\bitv1\b",re.I); STV_RE=re.compile(r"\bstv\b",re.I); UTV_RE=re.compile(r"\butv\b",re.I)
...
def normalise_channel(channel):
 ...
 return c
def download(url):
 req=urllib.request.Request(url,headers={"User-Agent":"SportTVGuide/0.9"})
...
def classify(title,desc,channel,categories):
 ...
 if PROMO_RE.search(tl) and not any(t in tl for t in ['premier league','efl championship','rugby','boxing','ufc','tennis','golf','cricket','formula','grand prix']):return None,0.0
...
def main():
 ...
 (OUT/'health.json').write_text(json.dumps({'checked_at':now.isoformat().replace('+00:00','Z'),'window_start':start.isoformat().replace('+00:00','Z'),'window_end':end.isoformat().replace('+00:00','Z'),'sources':health,'channel_count':len(cc),'sport_counts':dict(sorted(sc.items()))},indent=2),encoding='utf-8');print(json.dumps(health,indent=2))
