...
def fetch_tsdb(date_str):
    ...
    for league_name,league_id in TSDB_LEAGUES.items():
        ...
        try:
            req=urllib.request.Request(url,headers={"User-Agent":"SportTVGuide/0.6"})
...
def fetch_fixtures(date_str):
    ...
    if not espn_ok: out.extend(fetch_tsdb(date_str))
    return out
...
def main():
    ...
    for event in data.get("events",[]):
        ...
        if pair: home,away=pair
            ...
            if len(candidates)==1: home,away=canonical_team(candidates[0]["home"]),canonical_team(candidates[0]["away"])
            else:
